...
$Host.UI.RawUI.WindowTitle = 'AviumUI 一键刷机 - Y700 2023 (asphalt)'
...
# 刷机包(可能是分卷)
$zip = Find-File 'AviumUI-*.zip'
if (-not $zip) { $zip = Merge-Parts }
if (-not $zip) {
    Die '找不到刷机包' @(
        '请把 AviumUI-*.zip 放到脚本目录或 images 子目录下',
...
while ($true) {
    Title '请选择要做什么'
    W ''
    W '    1  完整刷入 AviumUI        (刷 Recovery → 清数据 → 刷系统)' 'White'
